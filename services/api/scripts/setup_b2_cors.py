...
existing CORS rules are preserved and one rule (ID `carla-lake-direct-upload`) is
...
RULE_ID = "carla-lake-direct-upload"
...
def _client():
    # Standalone client (not app.repo.get_s3_client) on purpose, but it uses the
    # same standardized settings: the region comes from B2_REGION (never a
    # hardcoded literal) and the endpoint is derived from it.
    return boto3.client(
        "s3",
        ...
        config=Config(signature_version="s3v4", user_agent_extra="b2ai-carla-sensor-data-lake"),
    )
